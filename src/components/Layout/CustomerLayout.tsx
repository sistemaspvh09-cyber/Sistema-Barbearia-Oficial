    ...
    <div className="min-h-screen bg-background text-on-surface flex flex-col font-body max-w-md mx-auto shadow-2xl relative overflow-x-hidden border-x border-white/5">
      ...
      <header className="sticky top-0 z-50 bg-[#0D0D0D]/90 backdrop-blur-xl border-b border-white/10 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {!isHome && (
            <button onClick={() => navigate(-1)} className="p-2 -ml-2 text-on-surface-variant hover:text-white transition-colors">
              <ArrowLeft size={20} />
            </button>
          )}
          <div>
      ...
      <nav className="fixed bottom-0 w-full max-w-md bg-[#0D0D0D]/90 backdrop-blur-xl border-t border-white/10 px-6 py-4 z-50 pb-safe">
        <div className="flex justify-between items-center max-w-xs mx-auto">
          <button onClick={() => navigate('/app')} className={`flex flex-col items-center gap-1 transition-all ${isHome ? 'text-[#C8FF00]' : 'text-on-surface-variant hover:text-white'}`}>
          ...
          <button onClick={() => navigate('/app/historico')} className={`flex flex-col items-center gap-1 transition-all ${location.pathname.includes('historico') ? 'text-[#C8FF00]' : 'text-on-surface-variant hover:text-white'}`}>
          ...
          <button onClick={() => navigate('/app/login')} className={`flex flex-col items-center gap-1 transition-all ${location.pathname.includes('login') ? 'text-[#C8FF00]' : 'text-on-surface-variant hover:text-white'}`}>
